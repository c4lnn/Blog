...
**[BZOJ 2301. Problem b](https://darkbzoj.tk/problem/2301)**

> 求 $\sum_{i=a}^{n}\sum_{j=b}^{m}[\gcd(i,j)=k]$。

根据容斥原理：

$\begin{aligned}
& \sum_{i=a}^{n}\sum_{j=b}^{m}[\gcd(i,j)=k] \\
= & \sum_{i=1}^{n}\sum_{j=1}^{m}[\gcd(i,j)=k]-\sum_{i=1}^{a-1}\sum_{j=1}^{m}[\gcd(i,j)=k]-\sum_{i=1}^{n}\sum_{j=1}^{b-1}[\gcd(i,j)=k]+\sum_{i=1}^{a-1}\sum_{j=1}^{b-1}[\gcd(i,j)=k]\end{aligned}$
...
**[SPOJ LCMSUM](https://www.spoj.com/problems/LCMSUM/en/)**
